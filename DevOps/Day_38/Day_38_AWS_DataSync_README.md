...
Transfer files from **Amazon S3** to **Amazon EFS** using **AWS DataSync**, then verify the transfer from an EC2 instance mounted to EFS.
...
Created a private S3 bucket and used it as the source for transfer.

![Private S3 Bucket](./Demo/01-s3-private-bucket-created.png)

### 2) Get DataSync Agent AMI

Used AWS Systems Manager Parameter Store to get the latest DataSync Agent AMI.
...
Initial transfer failed because EFS was not allowing NFS traffic from the DataSync Agent.
...
Created the transfer task successfully.

![Task Created](./Demo/12-datasync-task-created.png)

### 12) Start Task

Started the DataSync task with default settings.
